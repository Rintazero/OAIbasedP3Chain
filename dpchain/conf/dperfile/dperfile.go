...
import (
	"autoScript/conf/iniObject"
	iniobject "autoScript/conf/iniObject"
	jsonconfig "autoScript/conf/jsonConfig"
	"autoScript/conf/utils"
	"fmt"
	"os"
	"strconv"
	"strings"
...
func (didp *DperInit) CreateRootDir(rootDir string) {
	...
	if didp.ContractSetting.ContractMode {
		...
		contractExecDest := rootDir + string(os.PathSeparator) + didp.ContractSetting.ContractExecFileName + ExecutableSuffix
...
func (didp *DperInit) ConfigDper() {
	...
	for index, dperDir := range didp.DperDir {
		...
		if didp.ContractSetting.ContractMode {
			...
			execFile := "." + string(os.PathSeparator) + ".." + string(os.PathSeparator) + didp.ContractSetting.ContractExecFileName + ExecutableSuffix
			var arg1 string
			var arg2 string
